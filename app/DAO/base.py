...
class BaseDAO(Generic[T]):
    ...
    async def find_all(self, filters: BaseModel | None = None):
        ...
        try:
            ...
            logger.info(f"Найдено {len(record)} записей по фильтрам: {filter_dict}")
            return record
        except SQLAlchemyError as e:
    ...
    async def add(self, values: BaseModel):
        ...
        logger.info(f"Добавления записи в {self.model.__name__} с параметрами: {values_dict}")
        try:
            new_instance = self.model(**values_dict)
            self._session.add(new_instance)
            logger.info(f"Запись успешно добавлена в {self.model.__name__}")
    ...
    async def add_many(self, instances: List[BaseModel]):
        val_list = [item.model_dump(exclude_unset=True) for item in instances]
        logger.info(f"Добавление нескольких записей {self.model.__name__}. Количество: {len(val_list)}")
        try:
            new_instances = [self.model(**values) for values in val_list]
            self._session.add_all(new_instances)
            logger.info(f"Успешно добавлено {len(new_instances)} записей.")
            await self._session.flush()
            return new_instances
        except SQLAlchemyError as e:
            logger.error(f"Ошибка при добавлении записей")
            raise

    async def update(self, filters: BaseModel, values: BaseModel):
        filter_dict = filters.model_dump(exclude_unset=True)
        val_dict = values.model_dump(exclude_unset=True)
        logger.info(
            f"Обновление записей {self.model.__name__} по фильтру: {filter_dict} с параметрами: {val_dict}")
        try:
            query = (sqlalchemy_update(self.model).where(
                *[getattr(self.model, k) == v for k, v in filter_dict.items()]).values(**val_dict).execution_options(
        ...
        try:
            ...
            for record in records:
                ...
                if "id" not in record_dict:
                    continue
                updated_data = {k: v for k, v in record_dict.items() if k != "id"}
                stmt = (sqlalchemy_update(self.model).filter_by(id=record_dict['id']).values(**updated_data))
